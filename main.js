...
let model;
let initialModelPosition = { x: 0, y: 1, z: -10 };
let time = 0;
const movementAmplitude = 1;
const movementSpeed = 0.003; // Speed of the movement cycle
...
loader.load(
  ...
  function (gltf) {
    model = gltf.scene;
    model.position.set(
      initialModelPosition.x,
      initialModelPosition.y,
      initialModelPosition.z,
    );
    scene.add(model);
  },
  undefined,
...
function animate() {
  ...
  if (model) {
    ...
    model.position.z = initialModelPosition.z +
      movementFactor * movementAmplitude;
    ...
    veilMaterial.opacity = minVeilOpacity +
      backPosition * (maxVeilOpacity - minVeilOpacity);
